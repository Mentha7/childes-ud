...
The two most commonly seen tiers are \texttt{\%mor} and \texttt{\%gra} tiers \sidenote{As shown in Listing \ref{lst:chatsent2}.}, which can be generated automatically by the use of a series of CLAN programs \sidenote{MOR, POST, POSTMORTEM and GRASP}.
...
Since CHAT is more encompassing than CoNLL-U and CoNLL-U files have no exclusive information that require previously non-exist tiers or headers, data contained in CoNLL-U files can be seen as a subset of that of the CHAT files. The general information types contained in CHAT files are summarised in \Cref{tab:info}. It is clear from the table that file- and sentence-level meta data distributed in diverse places of a CHAT file are all stored as sentence comments in CoNLL-U.
